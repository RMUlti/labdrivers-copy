...
class Keithley2400old:
    ...
    def __init__(self, GPIBaddr):
        ...
        try:
            ...
            assert (
                len(self._visa_resource.query("*IDN?")) > 0
    ...
    def _pullData(self):
        ...
        buffer_points = self._visa_resource.query_ascii_values("TRACE:POINTS:ACTUAL?")[
            0
        ]
    ...
    def setSourceSweep(self, source, startValue, stopValue, sourceStep):
        ...
        if source.lower() == "voltage":
            ...
            logger.error(
                "Sweep not configured. Expected source to be one of ['voltage' | 'current']."
            )
    ...
    def setMeasure(self, measure, senseMode=0):
        ...
        if measure.lower() == "voltage":
            ...
            logger.error(
                "Measurement type not set. Expected one of ['voltage' | 'current' | 'resistance']"
            )
    ...
    def setCompliance(self, source, limit):
        ...
        if source.lower() == "current":
            ...
            logger.error(
                "Compliance limit not set. Expected one of ['voltage' | 'current']"
            )
    ...
    def getMeasure(self):
        ...
        query_result = (
            self._visa_resource.query("SENSE:FUNCTION?").split(",")[-1].strip('"')
        )
    ...
    def getSource(self):
        ...
        if source == "VOLT":
            return ("voltage", self._visa_resource.query("SOURCE:VOLTAGE:LEVEL?"))
        elif source == "CURR":
            return ("current", self._visa_resource.query("SOURCE:CURRENT:LEVEL?"))
    ...
    def saveData(self, filePath, fileName="test.csv", mode="i"):
        ...
        # create new files with incrementing filenames, e.g. filename0001.csv
            ...
            while True:
                ...
                incrementalFileName = (
                    fileName.rstrip(".csv") + "{:04d}".format(saveCounter) + ".csv"
                )
    ...
    def setFilter(self, value, count=10):
        if str(value).lower() == "on" or str(value).lower() == "off":
            self._visa_resource.write("SENS:AVER " + str(value).upper())
        else:
            return "Use 'ON' or 'OFF' as the first input."

        if int(count) > 0 and int(count) < 101:
            self._visa_resource.write("SENS:AVER:COUN " + str(count))
        else:
            return "Count argument must be between 1 and 100."
